...
This directory contains files for setting up a Caddy docker container for **local tests only!**
 ...
 Simply run the [docker-compose file](docker-compose.yml) inside this directory to start Caddy and access the web-app on [localhost:8080](http://localhost:8080). Note that this Caddy is exposed to the whole local network in order to make it accessable to mobile devices as well. 
